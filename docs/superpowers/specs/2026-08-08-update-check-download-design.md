...
- `checkUpdate()`：拉取 version.json → 解析 → 取本平台 `files` 条目（`no-installer` 若缺失）→ 用条目内 `version` 与当前版本比较 → 返回结果
- `downloadUpdate()`：流式下载到下载目录 → SHA256 校验 → 打开安装包
...
```json
{
  ...
  "files": {
    ...
    "mac-arm64": { "version": "0.3.0", "url": "https://updates.zhipengcoding.com/BulletChat-0.3.0-mac-arm64.dmg", "sha256": "…" },
...
- `notes` 取最近一次发布的更新说明；平台版本不同步时说明以最新发布为准（自用可接受）
- `sha256` 必填，下载后校验，防篡改/损坏
- 版本比较：数字逐段比较（`0.9.0 < 0.10.0`），非法版本视为检查失败
...
「忽略此版本」→ 版本号写入 config（`ignoredUpdateVersion`，经 configCore）。之后检查跳过该版本，直到出现更高版本才重新提醒。

### 4.5 下载

- 位置：`app.getPath('downloads')`；不可写时回退 `userData` 临时目录
- 文件名：version.json 中 URL 尾部文件名（如 `BulletChat-0.2.0-win-x64.exe`），防重复加序号
- 流式下载，进度事件推送；可取消（中止流、删半成品）
- 完成后校验 SHA256 → 通过则 `shell.openPath()` 打开安装包 → 通知「下载完成」
- 失败/校验失败：删文件，设置页提示重试
...
Nginx 配置要点（静态站点）：根目录指向 DEPLOY_PATH，SSL 用现有证书，开启 `location /` 静态文件服务；`.conf` 写入 `/www/server/panel/vhost/nginx/updates.zhipengcoding.com.conf`，含 SOP 9.6 的 well-known/extension include 与证书配置。
...
```bash
# macOS 电脑上发布 mac 包
node tools/publish-update.js --platform mac --notes "修复 xxx"
# Windows 电脑上发布 win 包
node tools/publish-update.js --platform win --notes "修复 xxx"
```

流程（仅操作当前平台，不影响另一平台）：

1. 校验本地 `dist/` 下有当前平台对应版本的产物（mac → dmg，win → exe）
2. 计算产物 SHA256（node:crypto，跨平台一致）
3. **拉取远程 version.json**（`https://updates.zhipengcoding.com/version.json`）合并：读不到（首次发布/网络失败）则新建
   - 更新本平台 `files` 条目（version/url/sha256），**保留另一平台条目不变**
   - 顶层 `version` = 所有平台条目的最大版本
   - `notes` = 本次发布的说明
4. 写 `dist/updates/version.json`，上传至服务器静态站点根目录
5. 服务器端 `chown www:www` 保证可读

**上传后端自动选择**：优先 `rsync`（macOS 自带）；Windows 无 rsync，回退 `scp`（Windows 10+ 自带 OpenSSH）。SSH 连接参数沿用 SOP 模式，从 `deploy.env` 读取（`DEPLOY_HOST` / `DEPLOY_USER` / `DEPLOY_SSH_KEY` / `DEPLOY_PATH`）。

日常发版：各平台在自己电脑上 `npm run build:<平台>` 后执行对应 `--platform` 的发布命令。
...
- 版本比较：`0.1.0 < 0.2.0`、`0.9.0 < 0.10.0`、相等、非法输入；客户端以本平台条目 `version` 为准（双平台版本不同步时不循环提示）
- 平台选择：darwin+arm64 → mac-arm64；win32+x64 → win-x64；未知 → null
- 忽略版本：忽略后同版本不提醒、更高版本重新提醒
- SHA256 校验：已知内容对已知哈希
- **发布脚本合并逻辑**：远程有 version.json 时只更新本平台条目、保留另一平台；顶层 version = 各平台最大值；远程读不到时新建
- 下载流程：本地 `node:http` 临时 server 模拟 version.json + 安装包，注入 fetch 测完整下载、校验、取消、失败清理
...
| `tools/publish-update.js` | 新增，发布脚本（`--platform mac|win` 单平台发布、远程合并、rsync/scp 自动选择、读 deploy.env） |
