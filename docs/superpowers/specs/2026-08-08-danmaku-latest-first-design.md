...
### S3 渲染层最新优先 + 视觉源头降产（实机验证补强）

**背景（2026-08-08 冒烟实测）**：脑侧修复生效（轨道空闲时文字回复 0.38s 上屏），但活跃期渲染层成新瓶颈——舞台仅 5 轨道 × 8s 动画 ≈ 0.625 条/s 容量，视觉 5 条/10s（0.5/s）已占 80%，叠加文字回复后总生成速率 > 容量，danmaku.js 的 pending 队列（FIFO 无上限）实测 30 秒涨到 28 条，文字回复排队后上屏延迟恶化到 32.7s。用户否决"丢弃视觉"（= 视觉识别白做 + 调用白花钱）。

**方案**：
- S3-1 **视觉源头降产（不丢不浪费）**：`generateVision` 的 `maxCount` 从 `replyCount` 改为 `Math.min(replyCount, VISION_MAX_COUNT)`，`VISION_MAX_COUNT = 3` 常量。视觉识别照常（每 10s 评论一次屏幕变化），产出速率降到 ~0.3/s，总速率 ~0.5/s < 舞台容量 → 正常负载零丢弃；AI 输出 token 减少 = 每次调用省钱。`parseDanmakuJson` 的解析上限仍用 replyCount（模型产出已 ≤3，无影响）。
- S3-2 **渲染层批次插队（两通道统一）**：brain 每次 burst 递增 `burstId`，emit meta 携带 `{ source: 'ai', burst: id }`（同批共享 id）。danmaku.js 的 `pending` 从"单条队列"改为"批次队列"：新到的批 `unshift` 队首（批内顺序保持），dequeue 从队首批取第一条；上限 `PENDING_LIMIT = 15` 条，超限从队尾丢最旧批（与脑侧"队首=最新、队尾=最旧、丢最旧"完全同构）。本地模式（无 burst 字段）每条自成一批，同样最新优先。
- 测试：brain.test.js 新增——批标记（同批共享 id、不同批递增）、视觉 maxCount（replyCount=10 → 3；replyCount=2 → 2）。渲染层无单测（项目惯例），验证靠 CDP 实机压力测试。

**不做**：不改文字 replyCount/burst 配置默认值；不区分文字/视觉优先级（统一最新优先，用户拍板）；渲染层队列上限仅在极端突发时触发。
